...
function Cab() {
  ...
  async function fetchCabDetailsAndTripDetails() {
    try {
      const response = await axios.get(
  ...
  returnDate(
    <div id="cjs">
      <div id="cabClass">
        ...
        <div id="cc">
          <form className="formclass" onSubmit={fetchCabDetailsAndTripDetails}>
        ...
        {cabs.length > 0 ? (
          cabs.map((cab, index) => (
            <div key={index} className="cabDetail">
              ...
              <button onClick={(e) => handleBooking(cab.cabId)}>SELECT</button>
            </div>
          ))
        ) : (
