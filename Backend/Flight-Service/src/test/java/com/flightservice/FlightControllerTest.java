...
public class FlightControllerTest {
    ...
    public void testGetAllFlight() throws Exception {
        ...
        mockMvc.perform(get(FlightConstant.FLIGHT + FlightConstant.GET_ALL_FLIGHT))
    ...
    public void testCreateFlight() throws Exception {
        ...
        mockMvc.perform(post(FlightConstant.FLIGHT + FlightConstant.SAVE_FLIGHT)
    ...
    public void testBookFlight() throws Exception {
        ...
        when(flightService.bookFlight(anyLong(), any(FlightTravellerFlightSeats.class), anyString())).thenReturn(bookingDetails);
        ...
        mockMvc.perform(post(FlightConstant.FLIGHT + FlightConstant.BOOK_FLIGHT_BY_FLIGHT_ID, 100001L, "user1")
               .contentType(MediaType.APPLICATION_JSON)
               .content(objectMapper.writeValueAsString(ftfs)))
               .andExpect(status().isCreated())
               .andExpect(jsonPath("$.flightBookingId").exists());
    }
    ...
    public void testGetFlightBookingDetailsByFlightBookingId() throws Exception {
        ...
        mockMvc.perform(get(FlightConstant.FLIGHT + FlightConstant.GET_FLIGHT_BOOKING_DETAILS_BY_FLIGHT_BOOKING_ID, 100001L))
               .andExpect(status().isOk())
               .andExpect(jsonPath("$.flightBookingId").value(100001L));
    }
    ...
    public void testDeleteFlight() throws Exception {
        ...
        mockMvc.perform(delete(FlightConstant.FLIGHT + FlightConstant.DELETE_FLIGHT_BY_FLIGHT_ID, 100001L))
               .andExpect(status().isOk())
               .andExpect(content().string("Flight details are deleted"));
    }
    ...
    public void testGetAllCityNames() throws Exception {
        ...
        mockMvc.perform(get(FlightConstant.FLIGHT + FlightConstant.GET_ALL_CITY_NAMES))
               .andExpect(status().isOk())
               .andExpect(jsonPath("$.length()").value(2));
    }
    ...
    public void testPaymentStatusChangeByTrainBookingId() throws Exception {
        ...
        mockMvc.perform(get(FlightConstant.FLIGHT + FlightConstant.PAYMENT_STATUS_CHANGE_BY_FLIGHT_BOOKING_ID, 100001L))
               .andExpect(status().isOk())
               .andExpect(jsonPath("$.paymentStatus").value("Payment done"));
    }
    ...
    public void testGetAllAvailableFlights() throws Exception {
        ...
        when(flightService.getAllAvailableFlights(anyString(), anyString(), any(Date.class), anyString())).thenReturn(flights);

        mockMvc.perform(get(FlightConstant.FLIGHT + FlightConstant.GET_ALL_AVAILABLE_FLIGHT, "CityA", "CityB", "2023-01-01", "Economy"))
               .andExpect(status().isOk())
               .andExpect(jsonPath("$.length()").value(2));
    }
}
